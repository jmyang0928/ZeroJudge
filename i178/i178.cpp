...
int main()
{
    ...
    while (cin >> X >> Y)
    {
        ...
        // 計算小智每張手牌贏的組數-輸的組數
        for (int i = 0; i < X; i++)
            winX += vecWin[i] = (lower_bound(vecY.begin(), vecY.end(), vecX[i]) - vecY.begin()) - (Y - (upper_bound(vecY.begin(), vecY.end(), vecX[i]) - vecY.begin()));
        if (winX < 0 && Y < -winX)
            cout << -1 << endl; // 若小智(輸的組數-贏的組數)比小遙的牌數還多，則不可能完成任務，輸出-1。
        else
        {
