...
int main()
{
    ...
    {
        startIdx[0] = startNode[0] - 'a';
        startIdx[1] = startNode[1] - '1';
        endIdx[0] = endNode[0] - 'a';
        endIdx[1] = endNode[1] - '1';
        vector<vector<int>> bfs(8, vector<int>(8, -1));
        bfs[startIdx[0]][startIdx[1]] = 0;
        if (startIdx[0] != endIdx[0] || startIdx[1] != endIdx[1])
        { // 若起點等於終點，距離等於0，不須BFS走訪。
            while (!qu.empty())
                qu.pop();
            qu.push(startIdx);
            while (true)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    nextIdx[0] = curIdx[0] + mv[i];
                    nextIdx[1] = curIdx[1] + mv[i + 2];
